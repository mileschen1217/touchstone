...
**`touchstone:assay` (pre-contract interview instrument)** — the fused single-session interview crucible runs between explore and design-spec (ADR-0034/0035): the AI lays out doc-grounded falsifiable assumptions (bold pass unconditional; a term-vs-ledger conflict surfaces AS an assumption) and extracts the human's tacit intent (want-vs-should-want probe, non-yes taxonomy), accounts the four knowledge quadrants, routes every known unknown through the **unknown disposition** (probe now / flip-trigger bypass / deferred log; a structural fork — ≥2 viable approaches with durability stakes — is one fork case and produces an ADR with the human as bet-owner), and emits the **guardrail contract block** (head: scope/invariants/contract facts/out-of-scope; tail: acceptance-seam scenario skeletons; flip-trigger registry; deferred log) consumed by design-spec or a PRD+seams light contract. Honest ceiling: done = accounting-completeness; the interview narrows unknown-unknowns, never proves them zero — instrument validity is measured downstream by the deviation log. _Avoid_: making assay a chain skip-condition (proportionality lives inside it); retiring feedback lenses on interview coverage (fire-rate evidence only); asking the human architecture/API questions (responsibility reversal).
