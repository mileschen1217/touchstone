...
**user-story** — the external/user-faced contract (the need + situation: "As an actor, I want X, so that Y"). The **intensional pole** (F-need): recognizable as the user's need, deliberately under-specified for verification. **Conditionally present** — present when there is an external / non-technical / third-party audience; **folds into `requirement` when solo** (the two-party gap collapses, so a technical user reads requirements directly). _Avoid_: requiring a separate user-story layer for solo/internal dogfood features — it folds into the requirement there.
