...
The layers **above** the AC — the human "what" that ACs make checkable. **3-layer model along the intension–extension axis, host-agnostic.** The three layers are three points on one continuum (intension → extension): **user-story → requirement → AC**. (Evolution ledger: ADR-0033.)

**user-story** — the external/user-faced contract (the need + situation: "As an actor, I want X, so that Y"), carrying a stable `US-N` id (the anchor `traces-to:` points back to). The **intensional pole** (F-need): recognizable as the user's need, deliberately under-specified for verification. **Always produced (always-3-layer).** touchstone defaults to authoring user-story → requirement → AC; **solo/non-solo is a read-depth choice, NOT a production switch** — a solo reader reads the requirement directly, a non-solo reader also reads the story (sound because the story→requirement completeness floor is solo-independent — see § story→requirement completeness). _Avoid_: branching the production mechanism on solo/non-solo — produce all three layers, always.

**PRD** — the **want-layer** of the contract spine: `why` (one-sentence intention) + `US-N` user-stories + out-of-scope. In touchstone wiring the want-layer is **always-on and native to `design-spec`** — the `## User Stories` section IS the single canonical home of the PRD layer (ADR-0021 addendum). There is no separately-produced PRD artifact, no separate chained PRD step, and no cross-artifact mirror. A shareable external PRD is an **out-of-band human action** when a real external-share need arises; touchstone owns only the want-layer inside the canonical design-spec. _Avoid_: producing a separate PRD document as part of touchstone's wiring — the single canonical lives in the spec's `## User Stories` (ADR-0021).

**`touchstone:crucible` (front-end contract orchestrator)** — the single front-end skill that chains: solution-grounding exploration (repo-state grounding scoped by the stated intent, ADR-0023) → `touchstone:assay` (unconditional — the pre-contract interview; proportionality lives inside assay, never as a chain skip-condition; ADR-0035) → `/touchstone:design-spec` (always). The want-layer (why + US-N) is native to design-spec; no separate PRD-producer step exists in the chain (ADR-0021). The AI authors the whole contract spine (why + US-N user-stories + requirements + ACs) in one pass; the human accepts **once** at the contract. It **reimplements none** of its sub-skills and adds only the chaining and US-N id discipline. It **terminates at human accept** and hands the accepted contract to `anvil` (§ Build-phase vocabulary); it auto-invokes the **consolidated design-review gate** before its terminal human-accept (the 3→2 front-load, ADR-0026); it does **not** invoke build (that is the `anvil` phase). _Avoid_: inserting a separate PRD-producer step between assay and design-spec — the chain tail is assay → design-spec (ADR-0021); making crucible emit requirements itself (design-spec authors those); or carrying it past human accept into the gate/build.

**`touchstone:assay` (pre-contract interview instrument)** — the fused single-session interview crucible runs between explore and design-spec (ADR-0034/0035): it aligns human and AI on the whole topic across three arms — **vocabulary** (term sheet: self-contained working definitions, one source marker each; a term-vs-ledger conflict surfaces AS an assumption row), **maps** (doc-grounded falsifiable laydown with unconditional bold pass, presented laydown-first per the shared fragment `skills/_shared/inject/laydown-first-presentation.md`; tacit-intent extraction — want-vs-should-want probe, non-yes taxonomy; questioning closes only through a published predict round, a miss reopens the question), and **territory** (explore / grounded-claims wiring, no new mechanism) — accounts the four knowledge quadrants, routes every known unknown through the **unknown disposition** (probe now / flip-trigger bypass / deferred log; a structural fork — ≥2 viable approaches with durability stakes — produces an ADR with the human as bet-owner), evidences alignment with **consequence probes** (≥1 per load-bearing ruling, human-falsifiable, each traced to a table row), and terminates at the record's **consensus section** (Scope / Invariants / Contract facts / Out-of-scope, every entry `[trace:]`-tagged to a human-confirmed row) + flip-trigger registry + deferred log + readiness ruling — readiness = explicit yes + a zero-correction probe round (ADR-0037); the record implements the confirmed-facts source contract (§ its entry), whose consumer derives facts from Consensus rows and authors the seam/AC layer itself. Honest ceiling: done = accounting-completeness; the interview narrows unknown-unknowns, never proves them zero — instrument validity is measured downstream by the deviation log. _Avoid_: making assay a chain skip-condition (proportionality lives inside it); re-packaging the consensus into an intermediate contract-block handoff at the terminus (retired form, ADR-0037); asking the human architecture/API questions (responsibility reversal).

**confirmed-facts source** — the qualification contract for any source of human-confirmed facts a contract author consumes; single home `skills/_shared/inject/confirmed-facts-source.md` (three-part qualification, two citation-granularity levels, four never-silent trigger classes). Trust follows the human-confirmation evidence, never the source type: an assay record's consensus section and an epic index `## Foundation` are two implementations, not the qualifying condition. Consumers (design-spec, crucible's PRD+seams path) point at the contract and carry only their own delta. _Avoid_: calling it a seam ("acceptance seam" keeps its meaning); branching consume behaviour on which producer made the source.
...
**story→requirement completeness (feedforward, the rung ABOVE requirement→AC)** — "is the requirement set the RIGHT set for its user-stories?" It hits the **SAME floor** as requirement→AC semantic completeness (intension–extension / no finite ground-truth total) — so it is **NOT a new problem** and **reuses the same 3-part shape**, one rung up: (1) **generative step = recognition** (assay's intent extraction; out-of-band brainstorming for genuinely-unformed intent), NOT EP/BVA — there is no rule-domain to partition at the story rung, so the systematic Nagy-5 / EP-BVA challenge stays at requirement→AC; (2) **structural slice** = every `US-N` has ≥1 requirement with `traces-to: US-N` + zero unresolved `[NEEDS CLARIFICATION]` — **mechanized**: the structural-floor checker is extended to enforce it (parity with the requirement→AC structural check), with US-N stories mirrored into the design-spec so the check stays single-file. A deterministic floor here is cheap, doubles as a forcing function (the author must confront every US to satisfy `traces-to`), and is the structural guard that stops `/build` industrialising a silently-dropped story; (3) **semantic completeness = human** (PRD approval; no mechanical oracle). _Avoid_: putting an adversarial systematic challenge at the story rung — PRD-level "did we miss a requirement?" is recognition-handled, not partitioned.
...
The evidence-honesty vocabulary (origin: ADR-0033 ledger). The lens these terms describe is live in `code-review batch` and the `epic-driven-roadmap` close procedure.
...
**catch-attribution** — for a caught defect/gap: which locus caught it (a gate, a live probe, a human) vs which locus SHOULD have caught it earliest. A catch by the right locus at the right time is the system working; a **gate-miss** is the attribution anomaly — the catching locus is later than (or outside) the should-have locus. _Avoid_: treating "gate-miss" as its own artifact — it is a *judgment over* a catch, recorded in the catch-attribution ledger.

**catch-attribution ledger** — the machine-local, append-only record of attribution ANOMALIES only (gate-misses, including human catches), settled 2026-07-02 (self-evolving-workflow-loop grill). Populated primarily by an epic-close sweep over deterministic sources (session-transcript user messages incl. interrupt-correction pairs; git fix-chains; Evidence Reckoning outcomes; checker fire-log patterns), with best-effort in-session labels as pre-seeding — the sweep, not the label, is the mechanism (human labelling is agent-remembers-soft). Each entry classifies its gap per the spine's existing split: `missing-AC` (feedforward) vs `false-green` (feedback). _Avoid_: (1) recording legitimate gate-time findings here — those live in their review artifacts; duplicating them is a dual-home; (2) conflating with the **epic scope-change ledger** (different subject: contract-scope changes, not defect attribution) or m-aos's tool-pattern ledger (harness friction, not workflow defects).
...
**challenge-result** — the JSON attestation (`<spec-stem>.challenge.json`, schema `challenge-result/v3`) that an INDEPENDENT challenger (challenger_id ≠ author_id) ran the requirement→AC completeness challenge-pass over a spec: `author_id`, `challenger_id`, `input_digest` (attested surface), `normalizer_version`, `findings[]` — each finding tagged with `type` {coverage-gap|real-defect|refinement} + `provenance` {original|fix-induced} (the classification the bounded-review loop gates on). Validated by `scripts/check-challenge-result.py`; fails closed. _Avoid_: treating it as a coverage record — it carries clarification findings, not a verdict.
