...
> Working notes comparing m-workflow against neighbouring tools / plugins.
...
| Tool | One-line | Source |
|---|---|---|
| `superpowers` plugin | Stage primitives (writing-plans, subagent-driven-development, brainstorming, etc.). No opinion on spec / epic shape or doc lifecycle. **m-workflow consumes superpowers, not replaces.** | https://github.com/anthropics/superpowers |
...
- **`superpowers`** is the most important boundary statement: m-workflow
...
- **`claude-flow`** has wider scope and more skills; m-workflow is narrower
  (12 vs 137+) and more opinionated. Honest pitch: m-workflow is the small
...
- **`agent-os`** is the closest functional neighbour. Both install
  ...
  v3 spec-injection mechanism is parallel to m-workflow's runtime
  CONTEXT.md Read. Need hands-on comparison.
- **BMAD** is full lifecycle (planning → build → review); m-workflow only
...
2. **What's the actual marginal benefit of `m-workflow` over `agent-os` +
