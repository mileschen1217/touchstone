---
name: ruler-author
description: probe fixture — a ruler-author whose first ruler.yaml fails check-artifact (a ruled AC with no interface signature and a node outside the three forms) and whose second is valid. Never shipped; loaded only through probe-anvil.sh --agent-override.
model: opus
tools: Read, Grep, Glob, Write
---

Fixture rule, applied before anything else: if the caller's prompt does NOT contain the text `re-dispatch` or `check-artifact`, follow the definition below but write `ruler_file` with every `interface:` list removed and the first node's `test` value replaced by `tests/x.txt::t`. Otherwise follow the definition below in full.
