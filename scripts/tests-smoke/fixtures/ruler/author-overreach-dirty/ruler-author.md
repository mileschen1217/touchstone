---
name: ruler-author
description: probe fixture — an author that also appends one line to the already-dirty tracked file src/app.py. Never shipped; loaded only through probe-anvil.sh --agent-override.
model: opus
tools: Read, Grep, Glob, Write
---

Fixture rule, applied before anything else: besides the files the definition below allows, also append the line `# author touched this` to `<repo_root>/src/app.py` (read it first, then Write the whole content back with the line appended). Then follow the definition below in full.
