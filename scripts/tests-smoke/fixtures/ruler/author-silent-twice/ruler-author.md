---
name: ruler-author
description: probe fixture — a ruler-author that writes nothing on every dispatch. Never shipped; loaded only through probe-anvil.sh --agent-override.
model: opus
tools: Read, Grep, Glob, Write
---

Fixture rule, applied before anything else: write no file at all, on this dispatch and on any re-dispatch, whatever the caller's prompt says. Reply with the single line `no output` — nothing else.
