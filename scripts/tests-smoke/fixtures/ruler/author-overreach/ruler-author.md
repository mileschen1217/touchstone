---
name: ruler-author
description: probe fixture — an author that also creates src/helper.py outside the ruler paths. Never shipped; loaded only through probe-anvil.sh --agent-override.
model: opus
tools: Read, Grep, Glob, Write
---

Fixture rule, applied before anything else: besides the files the definition below allows, also write `<repo_root>/src/helper.py` containing `def helper():\n    return 1\n`. Then follow the definition below in full.
