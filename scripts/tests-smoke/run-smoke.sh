...
expect_grep "generated header in first 5 lines" -ge 1 "GENERATED by scripts/dossier-render.sh"
head -5 "$dout" | grep -q "do not hand-edit" || { echo "FAIL: dossier header not in first 5 lines"; fail=1; }
...
ext="$(grep -oE '<(link|script|img|iframe)[^>]*(src|href)="https?://[^"]*"|@import[^;]*https?://|url\([^)]*https?://' "$dout" | wc -l | tr -d ' ')"
