...
scripts/metrics/persist-dispatch.sh "$raw_codex_path" "$collection_dir" "design-review" "$model" "$started_at" "$ended_at"
# On CC-only fallback (codex_healthy=0): use the --no-codex form:
# scripts/metrics/persist-dispatch.sh --no-codex --fallback-reason "<reason>" "$collection_dir" "design-review" "$model" "$started_at" "$ended_at"
