...
name: design-review
kind: workflow
description: Pre-Build review gate for design documents (spec, plan, ADR).
...
verification-honesty** — passing one lens never discharges the other.
`design-spec` is pure authoring; "design-spec was run" ≠ "this gate passed".

## Scope

In scope by frontmatter `type:` or path: spec, plan, ADR (`**/specs/**`,
`**/plans/**`, `**/adr/**`); else reply "not in scope — specs / plans / ADRs
only" and exit. Normal subject: `status: accepted-candidate`
(crucible invokes pre-accept; accept promotes after a clean pass);
`accepted` → standalone re-review; `draft` → not gated.

## Phase 1 — Inject vocabulary

Read `${CLAUDE_PLUGIN_ROOT}/skills/_shared/config-resolver.md` and follow it.

**Injection discipline (once, for all fragments):** Read each in full from
`${CLAUDE_PLUGIN_ROOT}/skills/_shared/` and place **verbatim** in the reviewer
envelope — a lens named without its usable text is the known defect class; this
path wires its own fragments.

Unconditional:
1. `inject/live-bearing-predicate.md` + `inject/ac-coverage-honesty-principle.md` —
   append to `system_prompt` AND carry as `evidence_honesty_vocab`.
2. `skills/_shared/inject/design-soundness-honor-check.md` — prepend to
   ...
   the feedback duty.
3. `ground-and-sweep.md` — Read and inject verbatim into the reviewer envelope;
   the fragment carries the saturation definition and scope-resolution rule.
   Unit = each *emitted finding* (file / line / field / AC-id).

Conditional (`source-as-truth` in `bundle.disciplines`): read
`inject/bridge-content-gate.md` + `inject/standing-vs-transient-bridge.md`; set
...
reviewers; the human resolves first. Zero → proceed.

## Phase 3 — Dispatch

Invoke `Skill(skill: "touchstone:cross-provider-reviewer")`: `task` = full doc
text, `system_prompt` = prompt below + Phase-1 injections, `discipline_mode` per
Phase 1, `role` = `design-reviewer`, `task_dir` optional.

### Doc-review prompt (keep inline)
...
> aspiration; interfaces name fields, types, error returns; error handling maps
> to concrete scenarios; invariants are cross-cutting rules; a coined term is
...
> promised behaviour will be witnessed, hiding nothing: for EACH requirement,
...
> path — this hunts ACs that do NOT exist; the doc has a non-empty
...
> a declaration check, no test source exists yet; a standing-runtime feature
...
> severity (Critical, High, Medium, Low), each citing section + concrete fix.
> End with verdict: approve | revise | block, then the sentinel:
> `STAGE-REVIEW-SUMMARY: critical=<n> high=<n> degraded=<true|false>`

## Phase 4 — Apply findings

Sum Critical+High (C+H) across reviewers:
...
A ⚠️ DEGRADED / ⚠️ PARTIAL banner (orthogonal to C+H) → present it VERBATIM and
...
the metrics window (silent no-op on failure):
