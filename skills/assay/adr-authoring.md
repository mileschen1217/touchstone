...
Referenced by `/touchstone:assay` (structural-fork case) and `/touchstone:design-spec`. Not a standalone skill. The template is `<plugin-root>/docs/adr/template.md` (single home — do not restate its format here; copy it into the project's `docs/adr/` on first use).
...
1. Otherwise author directly into the public ledger:
   - Initialize `docs/adr/` on first use (with user consent), copying
     `<plugin-root>/docs/adr/template.md` in as `docs/adr/template.md`
