...
If contains `source-as-truth`:
  Read `${CLAUDE_PLUGIN_ROOT}/CONTEXT.md` § "Bridge content gate" AND § "Standing vs transient bridge".
...
If no `.claude/m-workflow.yaml` exists in the project: print one-line hint "Run /m-workflow:init to configure paths and disciplines." Continue in default mode (no vocab loaded).
