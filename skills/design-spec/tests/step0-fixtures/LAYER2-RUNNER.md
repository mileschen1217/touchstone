...
count: 8 design-spec + 3 epic-driven-roadmap (untouched by this refactor) = **11
fixtures total**, down from 18 historical / 16 pre-refactor. The
behavioral-class labels below are this file's own pre-refactor grouping (the
...
| Tier | What | Status |
|---|---|---|
| Structural Layer-1 | greps/awk over `skills/design-spec/SKILL.md`'s `^### Foundation & facts intake` region (rewritten anchor, post-P2) plus the epic-scaffold-side structural checks | **GREEN now** via `scripts/check-foundation-gate-structure.sh` — triggered by pre-commit checker |
