...
1. `sid="${CLAUDE_SESSION_ID:-${CLAUDE_CODE_SESSION_ID:?run inside a Claude Code session}}"` — the session whose runs you are reporting; it must match the OTel sink's `session.id`. (Claude Code exports the id as `CLAUDE_CODE_SESSION_ID`; the older `CLAUDE_SESSION_ID` is honoured first for back-compat.)
