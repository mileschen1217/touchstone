...
- **Producer ≠ judge:** the artifact is produced before the gate by a script/session/build run — never by the reviewer authenticating it. Authentication burden scales with fakeability: a hand-pasteable perf log demands strong provenance; a real `Agent()` transcript is largely self-attesting and demands less. Do NOT require cryptographic signing of artifacts (over-spec — a human is in the loop at close). Examples: real-device run → serial log; build session dispatching `Agent()` → subagent transcript.
