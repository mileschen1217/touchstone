...
- **Deviation entries.** Where what you build departs from the contract, write a `D-n` entry into the epic's `deviation.yaml` at once (the record anvil's deviation duty writes): it carries `phase`, `refs` (which AC/REQ it departs from), `gap` and `disposition` each as `{title, detail}`, and `derived: true` in place of refs only for a process-only note.
- **Every write under the epic dir re-renders the dossier** through the shipped hook; run the renderer yourself only when the hook did not fire.
