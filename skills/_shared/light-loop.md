...
- **Deviation entries.** Where what you build departs from the contract, write a `D-n` entry into the epic's `deviation.yaml` at once (the record anvil's deviation duty writes; the schema lists every field): with the schema's other fields it carries `phase`, `refs` (which AC/REQ it departs from), `gap` and `disposition` each as `{title, detail}`, and `refs: []` with `derived: true` only for a process-only note.
