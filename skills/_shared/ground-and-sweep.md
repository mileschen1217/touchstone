...
- **sweep-to-dry:** the work unit is the AC's true-subject full set; stop only at **saturation — the multi-channel plateau: the next orthogonal channel surfaces nothing new** (the *signal*, never a fixed channel count), and never at first-hit — first-hit is the named failure mode. Observing a plateau has a **floor of ≥2 orthogonal channels**: a zero-increment transition is only meaningful once ≥2 channels have run, so the ≥2 is a *precondition* to observe the plateau, NOT a stop-count — it is what forces a confirmation channel past a single-search first-hit. When fewer than 2 channels are available the plateau is unobservable and the sweep degrades to reach-under-determined (`reach-discovery.md`), never a fake plateau. The true-subject set spans **two orthogonal dimensions**, and a sweep is dry only when **both** are saturated:
...
Each arm's **unit identity and per-arm application** live in that consumer's wrapper (design-spec / design-review); the shared **plateau definition** (the saturation criterion above) lives here — one ruler, one home.
