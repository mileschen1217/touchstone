...
description: |
  ...
  epic under .touchstone/epics/<slug>/index.md. Invoke when: starting a new
...
`.touchstone/epics/<slug>/index.md`. Trackers are
...
1. Pick a slug — lowercase, hyphen-separated, names the deliverable surface
   (e.g. `port-statistics-stacking`), not a phase number.
2. Read the project's CLAUDE.md § Doc Routing for the concrete
   `.touchstone/epics/` path.
3. Read `templates/epic-index.md` and write `<epics-dir>/<slug>/index.md`
...
4. Add a row to `ROADMAP.md` § Active Epics (create the file from
   ...
   `| <slug> | <aim> | proposed | [index](<epics-dir>/<slug>/index.md) |`
