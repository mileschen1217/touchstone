...
1. **Status drift** — Check whether `ROADMAP.md` carries the GENERATED marker
   (`<!-- GENERATED by scripts/roadmap-render.sh`).
   - **Marker present (generated host):** skip row-level status comparison — the
     render script eliminates this drift class by construction. Report that ROADMAP
     is generated and drift-check was skipped.
   - **Marker absent (hand-maintained host):** every ROADMAP row's status must match
     the epic index frontmatter `status:`. Mismatch → finding.
