...
- Checker rail, run automatically by the hook on `git commit` / `git push`: pre-commit `check-plugin-graph.sh` (false edges, unwaived orphans, test-only nodes, stale waivers — entry set `.touchstone/checker/plugin-map.entries`, waiver ledger `.touchstone/checker/waivers.yaml`), `check-fixture-consumers.sh`, `check-lens-manifest.sh`, `check-manifest-counts.sh`; pre-push `check-plugin-ratchets.sh` (baseline `.touchstone/checker/baselines/plugin-ratchets.txt`; every ratchet reads a figure `plugin-map.sh` emits and none recomputes it — `max_stage_load_tokens` is defined over in-session contexts only, `arm_load_tokens` sums a stage's dispatched contexts, `conditional_load_tokens` holds the bytes of `load-when` files — commit a lower value when the checker prints `ratchet may fall`).
- **Plugin review (touchstone-local, never shipped):** when a diff changes shipped agent behaviour—skill or agent instructions, workflow routing, hook enforcement, or a script those surfaces execute—the maintainer runs `bash .touchstone/checker/standalone/plugin-review.sh <epic-dir> --reviewer <codex|claude-code>` **before** `/touchstone:deliverable-review`. Select one fresh reviewer independent from the builder; it runs all four lenses in `.touchstone/checker/standalone/plugin-review-rubric.md`. Pure packaging/version, fixture, checker, or reader-documentation changes do not trigger this gate unless they also change that behaviour. A second reviewer is optional escalation, not a discharge requirement; consider it when this review machinery changed, before overruling or waiving a disputed C/H, or when the owner requests it. The resulting `review.yaml` (`gate: plugin-review`) remains subject to the C+H = 0 push gate. Claude Code and Codex compatibility are separately established by each harness's validator and live smoke test, not by reviewer vendor count.
